...
import { useAcceptSupportTicket, useDeleteSupportTicket } from "@/hooks/mutations/use-support-mutations";
  ...
  const cols: Column<any>[] = [
    ...
    { key: "actions", header: "Actions", render: (r) => (
      <div className="flex items-center gap-2">
        ...
        <button className="rounded p-1.5 text-primary hover:bg-primary/10"><MessageSquare className="h-4 w-4" /></button>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-xl rounded-xl border border-border bg-card p-5 shadow-2xl">
        ...
        <div className="mt-4 flex justify-end"><button onClick={onClose} className="rounded-md border border-border px-4 py-2 text-sm hover:bg-accent">Close</button></div>
      </div>
    </div>
  );
}

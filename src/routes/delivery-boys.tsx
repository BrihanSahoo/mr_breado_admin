...
import { Bike, CheckCircle2, CreditCard, Download, Eye, FileText, IndianRupee, MapPin, ShieldCheck, Wallet, X } from "lucide-react";
  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-black/75 p-4 backdrop-blur-sm">
    <div className="mx-auto my-6 w-full max-w-6xl rounded-3xl border border-border bg-card shadow-2xl">
      ...
      {loading ? <div className="p-10 text-center text-muted-foreground">Loading rider account…</div> : <div className="space-y-6 p-5">
        ...
        {detail.verificationRequest?.documents?.length ? <section className="rounded-2xl border border-border p-4"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div><h3 className="text-lg font-bold">Verification documents</h3><p className="text-xs text-muted-foreground">Preview or download every document submitted by this rider.</p></div><StatusBadge status={detail.verificationRequest.status || detail.verificationStatus || "PENDING"} /></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{detail.verificationRequest.documents.map((d,i)=>{const url=String(d.url||d.downloadUrl||"");const label=String(d.alt||`Document ${i+1}`).replace(/([A-Z])/g," $1").trim();const isPdf=/\.pdf(?:$|\?)/i.test(url)||url.startsWith("data:application/pdf");return <div key={`${url}-${i}`} className="rounded-xl border border-border bg-background/60 p-3"><div className="mb-3 flex items-center gap-3"><div className="rounded-lg bg-primary/10 p-2 text-primary">{isPdf?<FileText className="h-5 w-5"/>:<Eye className="h-5 w-5"/>}</div><div className="min-w-0"><div className="truncate text-sm font-bold capitalize">{label}</div><div className="text-xs text-muted-foreground">{isPdf?"PDF document":"Image document"}</div></div></div><div className="flex gap-2"><button type="button" disabled={!url} onClick={()=>url&&setPreview({url,label})} className="flex-1 rounded-lg border border-primary/30 px-3 py-2 text-xs font-bold text-primary hover:bg-primary/10 disabled:opacity-40"><Eye className="mr-1 inline h-3.5 w-3.5"/>View</button><a href={url||undefined} download target="_blank" rel="noreferrer" className="flex-1 rounded-lg border border-border px-3 py-2 text-center text-xs font-bold hover:bg-muted"><Download className="mr-1 inline h-3.5 w-3.5"/>Download</a></div></div>})}</div></section>:<section className="rounded-2xl border border-dashed border-border p-6 text-center text-sm text-muted-foreground">No verification documents submitted yet.</section>}
      ...
      {preview && <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/85 p-4" onClick={()=>setPreview(null)}><div className="max-h-[92vh] w-full max-w-5xl overflow-hidden rounded-2xl border border-border bg-card shadow-2xl" onClick={e=>e.stopPropagation()}><div className="flex items-center justify-between border-b border-border p-4"><div><div className="font-bold capitalize">{preview.label}</div><div className="text-xs text-muted-foreground">Rider verification document</div></div><div className="flex items-center gap-2"><a href={preview.url} download target="_blank" rel="noreferrer" className="rounded-lg border border-border p-2 hover:bg-muted" title="Download"><Download className="h-4 w-4"/></a><button onClick={()=>setPreview(null)} className="rounded-lg border border-border p-2 hover:bg-muted"><X className="h-4 w-4"/></button></div></div><div className="h-[75vh] bg-black/20 p-3">{/\.pdf(?:$|\?)/i.test(preview.url)||preview.url.startsWith("data:application/pdf")?<iframe src={preview.url} title={preview.label} className="h-full w-full rounded-xl bg-white"/>:<img src={preview.url} alt={preview.label} className="h-full w-full rounded-xl object-contain"/>}</div></div></div>}
  </div>;
}
